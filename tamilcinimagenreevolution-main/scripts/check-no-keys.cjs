    ...
    if (e.isFile()) {
      ...
      try {
        ...
        if ((m = APIKEY_ASSIGN.exec(txt))) {
          // if the value is a placeholder like <YOUR_...> or the assignment uses process.env/process.argv, skip
          const val = m[2] || '';
          const fullMatch = m[0] || '';
          // get nearby context to see if this is in a CLI example or markdown
          const ctxStart = Math.max(0, (m.index || 0) - 80);
          const ctx = txt.slice(ctxStart, (m.index || 0) + 80);

          if (/process\.env|process\.argv/.test(fullMatch) || /vercel\s+env/.test(ctx) || /```/.test(ctx) || /`/.test(ctx)) {
            // assignment is dynamic (uses env/argv) or a CLI / markdown example — skip
          } else if (!/^<.*>$/.test(val) && val.length >= 8) {
